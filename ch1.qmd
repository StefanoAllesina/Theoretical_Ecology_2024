...
#| echo: false
ns <- 45
ts1 <- discrete_model(initial_condition = 0.5, 
                      fun = logistic_map, 
                      number_steps = ns, 
                      pars = list(r = 3.675)) %>% mutate(label = "x0 = 0.5")
ts2 <- discrete_model(initial_condition = 0.4999, 
                      fun = logistic_map, 
                      number_steps = ns, 
                      pars = list(r = 3.675)) %>% mutate(label = "x0 = 0.4999")

ggplot(bind_rows(ts1, ts2)) + 
  aes(x = time, y = x_t, colour = label) + 
...
$$
x = c_1 B \quad t = c_2 \tau
$$

Choosing $c_1 = 1/a$, and $c_2 = 1/b$, we obtain:
...
#| echo: false
#| warning: false
dt_lhs <- tibble(x = seq(0, 10, by = 0.01), y = x / (1+x^2), label = "l.h.s.")
pl <- ggplot(dt_lhs) + aes(x = x, y = y, colour = label) + geom_line(linetype = 1)
values_rho_gamma <- tibble(intercepts = c(0.5,0.2,0.383971,1), slopes = c(-1/20,-0.2/10,-0.0383971,-1/10), label = "r.h.s.", lty = as.factor(2:5))
pl + geom_abline(data = values_rho_gamma, aes(slope =slopes, intercept = intercepts, linetype = lty, colour = label)) + 
  scale_linetype_manual(values = 2:5) + theme_bw() + theme(legend.position = "none") + 
  geom_point(data = tibble(x = c(2, 4 + sqrt(11), 4 - sqrt(11), 0.204078, 0.437432, 4.78128,8.88908), colour = as.factor(c(1,1,1,2, 3,3, 4))) %>% mutate(y = x/(1+x^2), label = ""), aes(colour = colour)) + ylab(expression(rho))
```
...
#| echo: false
#| warning: false
get_sols_spruce <- function(rho){
  Real_part <- c(0)
  Imaginary_part <- c(0)
  rho <- as.complex(rho)
  z1 <- (10 * (45 - 109 * rho) * rho^2 + 
  3 * sqrt(3) * sqrt(rho^3 * (1000 + rho * (-2200 + rho * (-4970 + 10201 * rho)))))^(1/3)
  x1 <- 1/3 * (10 + (30-97 * rho) / z1- z1/rho )
  Real_part <- c(Real_part, Re(x1))
  Imaginary_part <- c(Imaginary_part, Im(x1))
  x2 <- 1/6 * (20 + (97 * rho- 30) * (1 + sqrt(3)*1i)/z1 + (1- sqrt(3) * 1i) * z1/ rho)
  Real_part <- c(Real_part, Re(x2))
  Imaginary_part <- c(Imaginary_part, Im(x2))
  x3 <-1/6 * (20 + (97 * rho- 30) * (1 - sqrt(3)*1i)/z1 + (1 + sqrt(3) * 1i) * z1/ rho)
  Real_part <- c(Real_part, Re(x3))
  Imaginary_part <- c(Imaginary_part, Im(x3))
  return(tibble(Real_part = Real_part, Imaginary_part = Imaginary_part, rho = Re(rho)))
}

solutions_spruce <- map_df(seq(0.25, 0.65, by = 0.001), get_sols_spruce) %>% filter(abs(Imaginary_part) < 10^-10, Real_part >= 0) %>% select(rho, Real_part) %>% 
  rename(x = Real_part) %>% mutate(stable = -2 * x /(1+x^2)^2 + rho - rho * x / 5)
ggplot(solutions_spruce, aes(x = rho, y = x, colour = as.factor(sign(stable)))) + geom_point() + theme_bw() + theme(legend.position = "none") + xlab(expression(rho))
```
...
- Robert M. May, 1976. [*Simple mathematical models with very complicated dynamics*](https://www.nature.com/articles/261459a0). Nature 261:459-467

This paper discusses how simple models relates to experimental data on population growth:

- Michael P. Hassell, John H. Lawton, Robert M. May, 1976. [*Patterns of dynamical behavior in single species populations*](https://www.jstor.org/stable/3886). Journal of Animal Ecology 45:471-486

The paper analyzing the dynamics of the spruce budworm:

- Donald Ludwig, D. D. Jones, Crawford S. Holling, 1978. [Qualitative Analysis of Insect Outbreak Systems: The Spruce Budworm and Forest](https://www.jstor.org/stable/3939?seq=1). Journal of Animal Ecology, 47:315-332
